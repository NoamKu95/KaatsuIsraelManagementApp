...
export function useComboBox(props, state) {
  ...
  listIds.set(state, menuProps.id); // By default, a KeyboardDelegate is provided which uses the DOM to query layout information (e.g. for page up/page down).
  ...
  let onBlur = e => {
    ...
    if (e.relatedTarget === buttonRef.current || (_popoverRef$current = popoverRef.current) != null && _popoverRef$current.contains(e.relatedTarget)) {
      return;
    }
